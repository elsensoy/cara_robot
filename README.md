...
| **U12 — continuous walk** | `walk.py`: precomputed periodic gait cycle played on a loop + a gated roll trim | ❌ **documented limit** — *fast* (`t_step` ≲ 4 s): completes the swing, then topples at the double-support transfer (lateral COM momentum > the 22.5 mm foot's ~1 N·m CoP — the U9 `balance_margin` wall); *slow* (`t_step` ≈ 8 s): stays upright but no forward drive (kinematic playback ≠ push-off). Needs a ZMP/capture-point controller or wider feet; U11 stepping stays Cara's locomotion |
| A dynamic gait controller / wider feet (U13) | — | ⬜ next |
