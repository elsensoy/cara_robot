...
| **U8 — lift one foot** | `lift_foot.py`: + raise the free foot 5–10 mm, hold ~1.5 s in single support, return to double support | ✅ **MET** — free foot fully unloaded, stance carries 100 % weight, COM +10 mm inside the stance polygon, clean return, both sides; torque bottleneck is the swing hip_roll, not the ankle |
...
| Path | What it is |
| ---- | ---------- |
| `cara_description/` | **Robot description + simulation model.** Composed YAML: `left_leg` → `cara_lower_body` (mirror + floating pelvis) → `cara_full_body` (`+ include cara_upper_body`) → generated URDF + MJCF. Validation + analysis scripts; frozen `baselines/` for regression. Currently: full body (4.43 kg) **standing, weight-shifting, and briefly standing on one foot** (lift 5–10 mm, hold, return) under PD + a minimal roll trim — torso / head / electronics / passive-arm / ear lumps added in stages (Phases U1–U6), then the first balance/control steps (U7 unload, U8 single-support). Built in stages. |
...
- **[`cara_description/docs/single_support_notes.md`](cara_description/docs/single_support_notes.md)** : past the morphology boundary — U7 unloading one foot + U8 the first single-support hold (lift 5–10 mm, hold, return), both with a minimal disclosed roll trim (→ U9 a real balance controller)
