...
| **U6 — full-body regression** | re-run the whole suite on the complete model; `subsystem_summary.py` builds it up one subsystem at a time vs the frozen baseline | ✅ **standing MET, weight-shift MET at ±0.020 m**; COM driven by torso+head, whole-body yaw inertia by the arms; morphology-validation phase (U1–U6) closed |
| **U7 — unload one foot** | the balance/control boundary; `unload_foot.py` shifts the COM onto one foot then unweights the other (foot held level) | ✅ **MET** — free foot ~0 N with the COM +13 mm inside the *stance* foot polygon, foot 1 mm off the ground, both sides (needs a minimal roll trim) |
| **U8 — lift one foot** | `lift_foot.py`: + raise the free foot 5–10 mm, hold ~1.5 s in single support, return to double support | ✅ **MET** — free foot fully unloaded, stance carries 100 % weight, COM +10 mm inside the stance polygon, clean return, both sides |
| **U9 — single-support balance** | `single_support.py`: swap the roll trim for a COM-feedback controller; 5 s hold + lateral-push sweep. `balance_margin.py` diagnoses the envelope | ✅ **MET** — 5 s one-foot hold (COM drift 2.3 mm, tilt 2.8°), rejects ~1 N·100 ms toward swing / ~3 N toward stance. `balance_margin.py`: the CoP moment budget is 0.98 N·m (71 % spent holding the pose) and the lifted leg parks the CoP 16 mm toward the inner edge of the 22.5 mm foot → 6.5 mm swing-side room; widening the sole to 30 mm ≈ doubles it. Structural limit, not the gains |
| **U10 — one deliberate step** | `step_once.py`: shift onto one foot, lift the other, swing it **forward** to a new foothold, place, transfer to a staggered stance with the pelvis advanced | ✅ **MET** — both legs leading, step 20–40 mm: foot placed within 7 mm, COM stays inside the support polygon, pelvis < 2.5°, stance slip < 4.2 mm, ends level (< 0.5°) with ~35 mm margin; COM advances ~half the step. Forward only (a sideways step is past the lateral envelope); quasi-static + U9's roll trim |
| **U11 — a short walk** | `gait.py`: chain the U10 step, alternating legs | ✅ **MET** — 4 quasi-static steps forward (110 mm, 24 mm stride), alternating legs, ends standing level (0.9°, 0.1 mm COM drift); steps 2–4 within 0.3 mm — periodic cycle (checked to 6). No new controller. `stride` ≲ 25 mm (swing reach); forward / straight / flat; *stepping*, not a dynamic gait |
| **U12 — continuous *kinematic* walk** | `walk.py`: precomputed periodic gait cycle played on a loop + a gated roll trim | ❌ **formulation wall** — *fast* (`t_step` ≲ 4 s): completes the swing, then topples at the double-support transfer; *slow* (`t_step` ≈ 8 s): stays upright but no forward drive (kinematic playback ≠ push-off). Not the hardware — see U13 |
| **U13 — reduced-order walking model** | `walk_model.py`: LIPM + capture point (`ẍ = ω₀²(x−p)`, `ξ = x + ẋ/ω₀`), pure Python; `--check` cross-checks ω₀ against MuJoCo | ✅ **a dynamic walk IS within Cara's morphology** — lateral limit cycle feasible for step times ≥ `T_min ≈ 0.22 s` (the 22.5 mm foot is enough), forward roomier (`a_x` = 45 mm); ω₀ = 5.7 /s, MuJoCo divergence fits 5.1 /s. The U12 wall was the formulation, not the feet |
| **U14 — DCM-tracking controller** | `dcm_walk.py`: plan CoP + footholds + DCM ref from U13, `p_cmd = p_ref + (1+k/ω₀)e` feedback, capture-point step adjustment | 🔶 **built, blocked** — doesn't yet walk. The planner is right (U13); realisation fails two ways: the from-rest first DCM (−41 mm) exceeds the ~20 mm double-support envelope (no lateral momentum to enter the limit cycle), and the position servos can't place the CoP fast enough. Needs **torque-controlled ankles** + a limit-cycle warm-start |
| Torque-controlled ankles + warm-start (U15) | — | ⬜ next |
...
| Path | What it is |
| ---- | ---------- |
| `cara_description/` | **Robot description + simulation model.** Composed YAML: `left_leg` → `cara_lower_body` (mirror + floating pelvis) → `cara_full_body` (`+ include cara_upper_body`) → generated URDF + MJCF. Validation + analysis scripts; frozen `baselines/` for regression. Currently: full body (4.43 kg) **standing, weight-shifting, and balancing on one foot for 5 s** (COM-feedback controller; rejects a small lateral push) — torso / head / electronics / passive-arm / ear lumps added in stages (Phases U1–U6), then the first balance/control steps (U7 unload, U8 single-support lift, U9 balance). Built in stages. |
...
- **[`cara_description/docs/single_support_notes.md`](cara_description/docs/single_support_notes.md)** : past the morphology boundary — U7 unloading one foot, U8 the first single-support hold, U9 a COM-feedback balance controller (5 s hold + lateral-push rejection; `scripts/balance_margin.py` diagnoses why the envelope is small), U10 one deliberate forward step (`scripts/step_once.py`), U11 a short quasi-static walk (`scripts/gait.py`: chains the U10 step, alternating legs — 4 steps, periodic cycle, ends standing), U12 a continuous-*kinematic*-walk attempt (`scripts/walk.py`) that topples at the double-support transfer, then U13 the reduced-order LIPM / capture-point model (`scripts/walk_model.py`) showing a **dynamic walk IS feasible** for Cara (lateral step time ≥ ~0.22 s) — the U12 wall was the formulation, not the feet; and U14 the DCM-tracking controller (`scripts/dcm_walk.py`: plan + capture-point feedback + step adjustment) — built and correct in structure, but not yet walking: it needs **torque-controlled ankles** to place the CoP and a limit-cycle warm-start
- **[`cara_description/docs/upper_body_notes.md`](cara_description/docs/upper_body_notes.md)** : the composed config hierarchy and the staged upper-body mass/inertia analysis (U1 torso, U2 head/neck, U3 electronics placement study, U4 passive arm masses, U5 ears + `I ~ m r²` head-inertia study, U6 full-body regression + per-subsystem summary table) measured vs a frozen baseline
